...
class FeesHelperTest {
    ...
    @Test
    void should_handle_fee_when_decision_hearing_fee_not_recognized() {

        when(asylumCase.read(
            DECISION_HEARING_FEE_OPTION,
            String.class
        )).thenReturn(Optional.of("unrecognized"));
        ...
        verify(asylumCase, times(0)).write(FEE_WITHOUT_HEARING, fee.getAmountAsString());
        verify(asylumCase, times(0)).write(FEE_WITH_HEARING, fee.getAmountAsString());
        verify(asylumCase, times(0)).write(PAYMENT_DESCRIPTION, "Appeal determined with a hearing");
        verify(asylumCase, times(0)).write(PAYMENT_DESCRIPTION, "Appeal determined without a hearing");
    }
    ...
    void should_throw_exception_when_fee_is_null() {
        ...
        assertThatThrownBy(() -> FeesHelper.findFeeByHearingType(feeService, asylumCase))
            ...
            .hasMessageContaining("Cannot retrieve the fee from fees-register.");
    }

    @Test
    void should_throw_exception_for_unrecognized_fee_type() {
